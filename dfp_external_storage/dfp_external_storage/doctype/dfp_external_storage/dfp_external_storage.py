...
class DFPExternalStorageFile(File):
	...
	def validate(self):
		#frappe.log_error("DFP - START VALIDATE")
	...
	def _is_moving_to_external(self):
		if not self.dfp_external_storage:
			return False
		
		# If dfp_external_storage is set but file_url still shows local path,
		# we're probably in the middle of moving the file
		is_local_url = self.file_url and self.file_url.startswith(('/private/', '/public/'))
		
		#frappe.log_error("DFP _is_moving_to_external", f"""
		#	dfp_external_storage: {self.dfp_external_storage}
		#	file_url: {self.file_url}
		#	is_local_url: {is_local_url}
		#	result: {bool(self.dfp_external_storage and is_local_url)}
		#""")

		return bool(self.dfp_external_storage and is_local_url)

	def _is_moving_to_local(self):
		#frappe.log_error("START is_moving_to_local")
		"""File is being moved FROM external TO local storage"""
		if self.is_new():
			#frappe.log_error("is_moving_to_local: is_new TRUE")
			return False

		if self.has_value_changed('dfp_external_storage'):
			#frappe.log_error("is_moving_to_local: has_val changed TRUE")
			old_external = self.get_value_before_save('dfp_external_storage')
			return old_external and not self.dfp_external_storage

		#frappe.log_error("is_moving_to_local: RETURNING FALSE")
		return False
	...
	def check_content(self):
		...
		#frappe.log_error(f"START check_content: ")
		if self._is_moving_to_local():
			# File is being moved back to local, content not available yet
			#frappe.log_error("check_content: is_moving_to_local: TRUE")
			return
		elif self.dfp_external_storage and not self._is_moving_to_external():
			#frappe.log_error("check_content: currently external, not being moved")
		...
		#d = inspect.getouterframes(inspect.currentframe())
		#frappe.log_error(f"ERRORERROR sending to super(check_content) {d}")
