...
class DFPExternalStorageFile(File):
    ...
    def validate_external(self):
        #put validation for s3 stuff here
        return True

    def validate(self):
        previous = self.get_doc_before_save()
        frappe.log_error(f"DFP VALIDATE PREV?: {previous} bool(previous): {bool(previous)}")
        #moving from normal -> external
        #frappe.log_error(f"DFP VALIDATE self.dfp_external_storage {self.dfp_external_storage} bool(self.dfp_external_storage) {bool(self.dfp_external_storage)} previous.dfp_external_storage {previous.dfp_external_storage} bool(previous.dfp_external_storage) {bool(previous.dfp_external_storage)}")
        ...
        #frappe.log_error(f"DFP VALIDATE SELF {self} self.dfp_is_s3_remote_file(): {self.dfp_is_s3_remote_file()} previous: {previous} previous.dfp_is_s3_remote_file(): {previous.dfp_is_s3_remote_file()} previous.dfp_external_storage {previous.dfp_external_storage} self.dfp_external_storage {self.dfp_external_storage}")
        return True if bool(self.dfp_external_storage) else super(DFPExternalStorageFile, self).validate()
